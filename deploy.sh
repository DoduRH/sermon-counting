...
echo "https://github.com/$(git config --get user.name | tr '[:upper:]' '[:lower:]')/$(git config --get remote.origin.url | rev | cut -d'/' -f1 | rev | sed 's/.git$//')/settings/pages"
echo ""
echo "Or visit the live site at:"
echo "https://$(git config --get user.name | tr '[:upper:]' '[:lower:]').github.io/$(basename $(git rev-parse --show-toplevel))"
echo ""
